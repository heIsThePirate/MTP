...
#define N_x 50 //Window Size in x-direction
#define N_y 50 //Window Size in y-direction
#define N_z 50 //Window Size in z-direction
#define endl '\n'

int main(void)
{
    fftw_complex in[N_x * N_y * N_z], out[N_x * N_y * N_z]; //declare the input and output complex arrays
    ...
    string inputFile = "function_values_3D"; //declare the input file
    ...
    for (i = 0; i < N_x; i++)
    {
        for (j = 0; j < N_y; j++)
        {
            for (k = 0; k < N_z; k++)
            {
                double p;
                sine_values >> p;
                in[k + N_z * (j + N_y * i)][0] = p; //Assign the REAL part of input
                in[k + N_z * (j + N_y * i)][1] = 0; //Assign the IMAGINARY part of input
            }
        }
    }

    string outputFile = "dft_3D"; //declare the output file
    ofstream dft;
    dft.open(outputFile); //open the file to print the output into

    //Forward Fourier transform, save the result in 'out'
    cout << "Calculating the forward fourier transform." << endl;
    p = fftw_plan_dft_3d(N_x, N_y, N_z, in, out, FFTW_FORWARD, FFTW_ESTIMATE);
    fftw_execute(p);
    ...
    for (i = 0; i < N_x; i++)
    {
        for (j = 0; j < N_y; j++)
        {
            for (k = 0; k < N_z; k++)
            {
                ...
                double a = out[k + N_z * (j + N_y * i)][0];
                double b = out[k + N_z * (j + N_y * i)][1];
                double c = (a * a + b * b); //Modulus
                double psd = c * c;         //Power Spectral density
                dft << k + N_z * (j + N_y * i) << " " << in[k + N_z * (j + N_y * i)][0] << " " << out[k + N_z * (j + N_y * i)][0] << " " << out[k + N_z * (j + N_y * i)][1] << " " << psd << endl;
            }
        }
    }
